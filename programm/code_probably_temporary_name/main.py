                        ...
                        if isinstance(port, Portal):  # поиск сопряжённого портала
                            if object_.who_id == port.my_id:  # проверка id портала
                                ...
                                if x_dir:  # если мы провели предыдущие проверки,то должно быть
                                    x_p, y_p = (port.rect.x - player.rect.x + x_dir,
                                                player.rect.y - port.rect.y)  # координаты, куда нужно всё переместить
    ...
    if objects:
        for object_ in objects:
            if isinstance(object_,
                          Land):  # вот чтобы фокусы Коперфилда не происходили очень сложно и мутурно считаем где игрок, куда подвинуть
                if player.mask.overlap(object_.mask,
...
if __name__ == '__main__':
    ...
    load_screen = pygame.transform.smoothscale(load_image('fons/load_screen.png'),
                                               (600, 600))  # подгрузка экрана загрузки, чтоб не делать это каждый раз
    oleg = pygame.mixer.Sound(
        '../assets/sounds/lever_sound.mp3')  # подгружаю этот звук, просто чтоб у меня был уже готовый объект для дальнейшего
    ...
    for level in range(1, len(os.listdir('../assets/levels')) + 1):  # из txt файла подгружаем уровени
        player, level_fon, level_sky, objects, player_objects, music, exit_ = parse_level(
            f'{level}_level')  # из функции отбираем все объеты
        indic = Indicator(main_window, exit_)  # рисуем кружочки в зависимости от кол-ва условий
        background = pygame.transform.smoothscale(load_image(f'fons/{level_fon}'),
                                                  (wight, height))  # растягиваем задник
                    ...
                    if event.key == pygame.K_RETURN:
                        for i in player_objects:  # на enter взаимодействие
                            if ((player.rect.x + player.rect.width // 2 + 70 > i.rect.x) and
                                ...
                                if not all(
                                        exit_.conditions):  # если нажатие не решило загадку, с шансом 1/12 олег скажет что-то грустное
                                    if randrange(12) == 4:
                                        oleg.stop()
                                        oleg = pygame.mixer.Sound(
